...
export const vehiclesRegisterValidation = z.object({
    ...
    registrationDate: z.coerce.date({
        required_error: "La date d'immatriculation est requise",
    }),
    ...
    numberOfSleepingPlaces: z.coerce.number()
    .int()
    ...
    length: z.coerce.string()
        ...
        .refine((val) => parseFloat(val) < 1000, {
            message: "La longueur doit être inférieur à 100",
        }),
    height: z.coerce.string()
        ...
        .refine((val) => parseFloat(val) < 1000, {
            message: "Le poids doit être inférieur à 1000",
        }),
    weight: z.coerce.string()
        ...
        .refine((val) => parseFloat(val) < 999999, {
            message: "Le poids doit être inférieur à 9999999",
        }),
    ...
    latCoordinates: z.string()
        .trim()
        .max(100, {
            message:
                "Les coordonnées de la ville ne doivent pas dépasser 100 caractères",
        }),
    lonCoordinates: z.string()
        .trim()
        .max(100, {
            message:
                "Les coordonnées de la ville ne doivent pas dépasser 100 caractères",
    ...
    insuranceNumber: z.string()
        ...
        .max(100, {
            message:
                "Le numéro d'assurance ne doit pas dépasser 100 caractères",
        }),
    insuranceExpirationDate: z.coerce.date({
        required_error: "La date d'expiration de l'assurance est requise",
    }),
    basePrice: z.number()
    ...
    equipmentIds: z.array(
        z.string()
            .uuid({ message: "ID d'équipement invalide" }),
    ),
    pictures: z.array(
        z.string()
            .trim()
...
export const vehiclesUpdateValidation = z.object({
    ...
    insuranceExpirationDate: z.coerce.date({
        required_error: "La date d'expiration de l'assurance est requise",
    }).refine((date) => date > new Date(), {message: "La date d'expiration doit être ultérieure à aujourd'hui"} ),
    basePrice: z.number()
        .min(0, { message: "Le prix journalier doit être positif" })
        .max(1000, { message: "Le prix journalier ne doit pas dépasser 1000€" })
        .transform((val) => val.toFixed(2)),
