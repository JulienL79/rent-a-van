import { Card, IVehicleCardPropsProfile } from "@molecules/Card";
import { useEffect, useState } from "react";
import { useAuthStore } from "@store/useAuthStore";
import { createVehicle, deleteVehicle, fetchVehiclesByUser } from "@api/vehicleApi";
    ...
    const prepareFormData = async () => {
        ...
        newFormData.fields = newFormData.fields.map(field => {
            if (field.id === "categoryId") {
                const options = categories?.data.map(cat => ({ label: cat.name, value: cat.id })) ?? [];
                return {
                    ...field,
                    options,
            ...
            if (field.id === "equipmentIds") {
                const options = equipments?.data.map(f => ({ label: f.name, value: f.id })) ?? [];
                return {
                    ...field,
                    options,
                    value: [],
