...
import { TAdminUserData, TAdminUserRow } from "../../../../types/User";
...
export const AdminUser = () => {
    ...
    const deleteUserById = async (id: string) => {
        try {
            ...
            handleError(err, "Une erreur est survenue lors de la suppression de l'utilisateur");
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (userResponses && userResponses.data) {
                ...
                const simplifiedData: TAdminUserRow[] = parsedData.map(({ id, email, roleName, createdAt }) => ({
                    id,
                    email,
                    roleName,
                    createdAt,
    ...
    if (id) {
        return (
            <User page="admin" id={id} onInteract={() => { }} />
        )
    }
    ...
    return (
        <DataTable pageData={{
            ...
            columns
        }} onSelect={selectUser} onDelete={deleteUserById} onUpdate={() => { console.log("Modif") }} />
    );
}