...
import { TAdminCategoryData, TAdminCategoryRow } from "../../../../types/Category";
import { deleteCategory, fetchAllCategories } from "@api/categoryApi";
...
export const AdminCategory = () => {
    ...
    const deleteCategoryById = async (id: string) => {
        try {
            ...
            handleError(err, "Une erreur est survenue lors de la suppression de la catégorie");
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (categoryResponses && categoryResponses.data) {
                const parsedData: TAdminCategoryData[] = categoryResponses.data.map((item) => ({
                    ...item,
                ...
                const simplifiedData: TAdminCategoryRow[] = parsedData.map(({ id, name }) => ({
                    id,
                    name
                }));
    ...
    if (id) {
        return (
            <Category page="admin" id={id} onInteract={() => { }} />
        )
    }
    ...
    return (
        <DataTable pageData={{
            ...
            columns
        }} onSelect={selectCategory} onDelete={deleteCategoryById} onUpdate={() => { console.log("Modif") }} />
    );
}