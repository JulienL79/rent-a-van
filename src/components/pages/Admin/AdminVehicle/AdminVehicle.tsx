...
import { TAdminVehicleData, TAdminVehicleRow } from "../../../../types/Vehicle";
...
export const AdminVehicle = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (vehicleResponses && vehicleResponses.data) {
                const parsedData: TAdminVehicleData[] = vehicleResponses.data.map((item) => ({
                    ...item,
                ...
                const simplifiedData: TAdminVehicleRow[] = parsedData.map(({ id, registrationPlate, createdAt }) => ({
                    id,
                    registrationPlate,
                    createdAt,
                }));
    ...
    if (id) {
        return (
            <Vehicle page="admin" id={id} onInteract={() => { }} />
        )
    }
    ...
    return (
        <DataTable pageData={{
            ...
            columns
        }} onSelect={selectVehicle} onDelete={deleteVehicleById} onUpdate={() => { console.log("Modif") }} />
    );
}