...
import { TAdminBookingData, TAdminBookingRow } from "../../../../types/Booking";
...
export const AdminBooking = () => {
    ...
    const deleteBookingById = async (id: string) => {
        try {
            ...
            handleError(err, "Une erreur est survenue lors de la suppression de la réservation");
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (bookingResponses && bookingResponses.data) {
                const parsedData: TAdminBookingData[] = bookingResponses.data.map((item) => ({
                    ...item,
                ...
                const simplifiedData: TAdminBookingRow[] = parsedData.map(({ id, createdAt, status, amount }) => ({
                    id,
                    createdAt,
                    status,
                    amount
    ...
    if (id) {
        return (
            <Booking page="admin" id={id} onInteract={() => { }} />
        )
    }
    ...
    return (
        <DataTable pageData={{
            ...
            columns
        }} onSelect={selectBooking} onDelete={deleteBookingById} onUpdate={() => { console.log("Modif") }} />
    );
}