...
import { TAdminEquipmentData, TAdminEquipmentRow } from "../../../../types/Equipment";
import { deleteEquipment, fetchAllEquipments } from "@api/equipmentApi";
...
export const AdminEquipment = () => {
    ...
    const deleteEquipmentById = async (id: string) => {
        try {
            ...
            handleError(err, "Une erreur est survenue lors de la suppression de l'équipement");
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (equipmentResponses && equipmentResponses.data) {
                const parsedData: TAdminEquipmentData[] = equipmentResponses.data.map((item) => ({
                    ...item,
                ...
                const simplifiedData: TAdminEquipmentRow[] = parsedData.map(({ id, name, icon }) => ({
                    id,
                    name,
                    icon
                }));
    ...
    if (id) {
        return (
            <Equipment page="admin" id={id} onInteract={() => { }} />
        )
    }
    ...
    return (
        <DataTable pageData={{
            ...
            columns
        }} onSelect={selectEquipment} onDelete={deleteEquipmentById} onUpdate={() => { console.log("Modif") }} />
    );
}