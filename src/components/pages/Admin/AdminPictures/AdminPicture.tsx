...
import { TAdminPictureData, TAdminPictureRow } from "../../../../types/Picture";
...
export const AdminPicture = () => {
    ...
    const deletePictureById = async (id: string) => {
        try {
            ...
            handleError(err, "Une erreur est survenue lors de la suppression de l'image");
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (pictureResponses && pictureResponses.data) {
                const parsedData: TAdminPictureData[] = pictureResponses.data.map((item) => ({
                    ...item,
                ...
                const simplifiedData: TAdminPictureRow[] = parsedData.map(({ id, src, userId }) => ({
                    id,
                    src,
                    userId
                }));
    ...
    if (id) {
        return (
            <Picture page="admin" id={id} onInteract={() => { }} />
        )
    }
    ...
    return (
        <DataTable pageData={{
            ...
            columns
        }} onSelect={selectPicture} onDelete={deletePictureById} onUpdate={() => { console.log("Modif") }} />
    );
}