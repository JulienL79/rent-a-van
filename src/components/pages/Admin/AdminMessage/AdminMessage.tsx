...
import { TAdminMessageData, TAdminMessageRow } from "../../../../types/Message";
...
export const AdminMessage = () => {
    ...
    const deleteMessageById = async (id: string) => {
        try {
            ...
            handleError(err, "Une erreur est survenue lors de la suppression du message");
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            if (messageResponses && messageResponses.data) {
                const parsedData: TAdminMessageData[] = messageResponses.data.map((item) => ({
                    ...item,
                ...
                const simplifiedData: TAdminMessageRow[] = parsedData.map(({ id, content, createdAt }) => ({
                    id,
                    content,
                    createdAt
                }));
    ...
    if (id) {
        return (
            <Message page="admin" id={id} onInteract={() => { }} />
        )
    }
    ...
    return (
        <DataTable pageData={{
            ...
            columns
        }} onSelect={selectMessage} onDelete={deleteMessageById} onUpdate={() => { console.log("Modif") }} />
    );
}