...
import { fuelTypeOptions, gearTypeOptions } from "@pages/Profile/ProfileVehicle/VehicleData";
...
export const Vehicle: React.FC<IVehicleProps> = ({ page, id, onInteract }) => {
    ...
    const toVehicleUpdatePayload = (vehicle: TVehicleDetails): VehicleUpdatePayload => {
        return {
            brand: vehicle.brand,
            model: vehicle.model,
            ...
            numberOfSleepingPlaces: vehicle.numberOfSleepingPlaces,
    ...
    const deleteVehicleById = async (id: string) => {
        try {
            ...
            handleError(err, "Une erreur est survenue lors de la suppression du véhicule");
    ...
    const prepareUpdateFormData = async (vehicle: TVehicleDetails) => {
        ...
        const onChangeSuggestion = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        newFormData.fields = newFormData.fields.map(field => {
            ...
            switch (field.id) {
                ...
                case "fuelType":
                    ...
                    break;
                case "equipmentIds":
                    options = equipments?.data.map(e => ({ label: e.name, value: e.id })) ?? [];
                    value = Array.isArray(payload.equipmentIds) ? payload.equipmentIds : [];
                    break;
                default:
            ...
            return {
                ...
                options,
                value,
                ...(field.kind === "suggestion" ? { onChange: onChangeSuggestion } : {}),
            };
        });
        ...
        return (
            <>
                <div className="vehicle-details">
                    ...
                    <div className="vehicle-section">
                        ...
                        <ul className="vehicle-specs">
                            ...
                            <li><strong>Consommation :</strong> {vehicle.consumption} L/100km</li>
                            <li><strong>Type de carburant :</strong> {vehicle.fuelType}</li>
                            <li><strong>Boîte de vitesse :</strong> {vehicle.gearType}</li>
                            <li><strong>Nombre de places assises :</strong> {vehicle.numberOfSeats}</li>
                            <li><strong>Nombre de couchages :</strong> {vehicle.numberOfSleepingPlaces}</li>
                            <li><strong>Dimensions :</strong> {vehicle.length}m (L) x {vehicle.height}m (H)</li>
                            <li><strong>Poids :</strong> {vehicle.weight} kg</li>
                        </ul>
                    </div>

                    <div className="vehicle-section">
                        <h2>Localisation</h2>
                        <p>{vehicle.cityName} ({vehicle.cityCode})</p>
                    </div>

                    <div className="vehicle-section">
                        <h2>Informations d’immatriculation</h2>
                        <ul>
                            <li><strong>Plaque :</strong> {vehicle.registrationPlate}</li>
                            <li><strong>Date d’immatriculation :</strong> {vehicle.registrationDate}</li>
                        </ul>
                    </div>
                    ...
                    <div className="vehicle-section">
                        <h2>Assurance</h2>
                        <ul>
                            <li><strong>Numéro :</strong> {vehicle.insuranceNumber}</li>
                            <li><strong>Expiration :</strong> {vehicle.insuranceExpirationDate}</li>
                        </ul>
                    </div>

                    {
                        vehicle.vehiclesToEquipments && vehicle.vehiclesToEquipments.length > 0 &&
                        <div className="vehicle-section">
                            <h2>Équipements</h2>
                        ...
                        <div className="vehicle-section">
                            ...
                            <ul className="price-periods">
                                {vehicle.pricePeriods.map((period) => (
                                    <li key={period.id}>
                                        <strong>{period.name}</strong> : du {period.startDate} au {period.endDate} (x{period.coefficient})
                                    </li>
                                ))}
                            </ul>
                        </div>
                ...
                <div className="button-group">
                    <Button className="primary-button" content="Modifier" onClick={() => setIsUpdatingVehicle(true)} />
