...
export const fetchCityByNameAndPostalCode = async (
    ...
    try {
        const response = await api.get<CitySuggestionResponse>(`/address/city/${content}`);
...
export const fetchCityCoordinates = async (code: string) => {
    try {
        const response = await api.get<{ message: string, data: {lat: string, lon: string} }>(`/address/coord/${code}`,
        );
        console.log(response);
        return response;
    } catch (err: any) {
